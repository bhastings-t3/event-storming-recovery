...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #lane.focusing .sticky.lo, #lane.focusing .inv.lo { opacity: .12; }
  #lane.focusing .edge-verb.lo { opacity: .06; }
  #lane.focusing svg.edges path.lo { opacity: .08; }
...
function layoutFlow(f, lane) {
  ...
  for (const o of insts) {
    if (o.kind === 'rm') { const a = instByIid[o.anchorIid], c = (above[o.anchorIid] = (above[o.anchorIid] || 0) + 1) - 1; o.x = a.x + c * 30; o.y = a.y - ROWH * (1.02 + c * 0.06); }
    else if (o.kind === 'inv') { const a = instByIid[o.anchorIid], c = (below[o.anchorIid] = (below[o.anchorIid] || 0) + 1) - 1; o.x = a.x; o.y = a.y + ROWH * (0.9 + c * 0.58); }
  }
  ...
  // place cards, measure, size canvas
  const PAD = 40, offX = PAD - Math.min(...insts.map(o => o.x)), offY = PAD - Math.min(...insts.map(o => o.y));
  const cardByIid = {};
  for (const o of insts) cardByIid[o.iid] = placeCard(o, lane, o.x + offX, o.y + offY, dupColor[o.id]);
  const box = {}; let maxR = 0, maxB = 0;
  for (const o of insts) { const e = cardByIid[o.iid], b = { x: e.offsetLeft, y: e.offsetTop, w: e.offsetWidth, h: e.offsetHeight }; b.cx = b.x + b.w / 2; b.cy = b.y + b.h / 2; box[o.iid] = b; maxR = Math.max(maxR, b.x + b.w); maxB = Math.max(maxB, b.y + b.h); }
