// The Event-Storming flow-board geometry engine, shared by BOTH explorers (issue #42):
//   - the live React SPA imports it via src/web/lib/layout.js (bundled by vite);
//   - the static generated explorer imports it via src/web/static-explorer/explorer-client.ts
//     (bundled by esbuild into dist/client/explorer-client.js, inlined into explorer.html).
// Both consumers are real modules under src/web, so this is a normal shared-module import — no
// codegen, no text-inlining (ADR-0007 made the static client a real module; ADR-0008 records this).
//
// It builds a flow's sticky lane by direct DOM construction and live measurement
// (offsetWidth/Height drive card placement and SVG edge routing), so it lives outside React and
// renders into a container element. The per-model bits the original relied on as globals
// (nodeById, PALETTE, selectedId, openDetail, and — SPA only — a right-click menu) are injected
// through `ctx = { nodeById, palette, selectedId, onOpenDetail, onContextMenu }`. Each consumer
// passes a ctx that reproduces its own behaviour: the SPA threads a real onContextMenu (its
// context menu) and the static explorer omits it (onContextMenu: undefined -> stays menu-less).
...
// Nodes/selectors that a background pan must NOT start on. The behavioural base set is common to
// both explorers; the SPA passes an extended list (its DM board + Flow/Data toggle add more
// interactive surfaces) via setupPanZoom's `panIgnore` param, so neither side is hardcoded here.
const BASE_PAN_IGNORE = '.sticky, .inv, .dupdot, .zoomctl, .ctx-title';
...
// swap in the new lane and re-fit. `panIgnore` is the selector a background pan must not start
// on; it defaults to the behavioural base set, and the SPA passes an extended list.
export function setupPanZoom(wrap, lane, ids, panIgnore) {
  if (wrap.__pz) { wrap.__pz.lane = lane; return wrap.__pz; }
  ids = ids || { zin: 'zin', zout: 'zout', zfit: 'zfit' };
  const panIgnoreSel = panIgnore || BASE_PAN_IGNORE;
  ...
  wrap.addEventListener('pointerdown', e => {
    if (e.button !== 0 || e.target.closest(panIgnoreSel)) return;
