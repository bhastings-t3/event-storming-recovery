...
function layoutFlow(f, lane) {
  ...
  // rank spine nodes by longest causal path (cap iterations to tolerate cycles)
  ...
  for (let it = 0; it < spine.length + 2; it++) {
    let ch = false;
    for (const e of spineEdges) if (rank[e.to] < rank[e.from] + 1) { rank[e.to] = rank[e.from] + 1; ch = true; }
    if (!ch) break;
  }
  const byRank = {};
