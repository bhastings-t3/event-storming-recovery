...
function layoutFlow(f, lane) {
  ...
  const spine = nodes.filter(n => !isRM(n) && !isInv(n));
  const spineIds = new Set(spine.map(n => n.id));

  // rank spine nodes by longest causal path. Flows contain cycles (e.g. a policy loop:
  // aggregate -> event -> policy -> command -> aggregate), so first strip the cycle-closing
  // back-edges via DFS — otherwise longest-path inflates ranks and scatters the graph.
  // The back-edges are still DRAWN later (as loop-back connectors); they just don't set rank.
  ...
  for (let it = 0; it < spine.length + 2; it++) {
    let ch = false;
    for (const e of dagEdges) if (rank[e.to] < rank[e.from] + 1) { rank[e.to] = rank[e.from] + 1; ch = true; }
    if (!ch) break;
  }
  // A non-actor node with no causal producer (e.g. an aggregate this flow only READS, like a
  // routing Team) defaults to rank 0 and lands among the actors. Pull it to the column where
  // it is actually referenced instead.
  const dagIndeg = {}; spine.forEach(n => (dagIndeg[n.id] = 0));
  for (const e of dagEdges) dagIndeg[e.to]++;
  for (const n of spine) {
    if (n.type === 'actor' || dagIndeg[n.id] > 0) continue;
    const nbRanks = (f.edges || []).filter(e => e.from === n.id || e.to === n.id)
      .map(e => (e.from === n.id ? e.to : e.from)).filter(x => spineIds.has(x) && x !== n.id).map(x => rank[x]);
    if (nbRanks.length) rank[n.id] = Math.max(0, Math.min(...nbRanks));
  }
  const byRank = {};
  spine.forEach(n => (byRank[rank[n.id]] ||= []).push(n));
  ...
  let minY = 0, maxY = 0;
  spine.forEach(n => { minY = Math.min(minY, pos[n.id].y); maxY = Math.max(maxY, pos[n.id].y); });

  const neighborsIn = id => (f.edges || []).filter(e => e.from === id || e.to === id).map(e => (e.from === id ? e.to : e.from)).filter(x => pos[x] && spineIds.has(x));

  // read models: band above the spine, x near their connected spine node(s); stagger only on x-collision
  const rms = nodes.filter(isRM);
  const rmSlots = [];
  rms.forEach(n => {
    const nb = neighborsIn(n.id).map(x => pos[x]);
    const x = nb.length ? nb.reduce((s, p) => s + p.x, 0) / nb.length : 0;
    let row = 0;
    while (rmSlots.some(s => s.row === row && Math.abs(s.x - x) < COLW * 0.9)) row++;
    rmSlots.push({ x, row });
    pos[n.id] = { x, y: minY - ROWH * (1.08 + row * 0.92) };
  });
  // invariants: band below their aggregate
  const invAggOf = id => { const e = (f.edges || []).find(e => e.to === id && e.verb === 'enforces'); return e ? e.from : null; };
  const invByAgg = {};
  nodes.filter(isInv).forEach(n => { const a = invAggOf(n.id); (invByAgg[a] ||= []).push(n); });
  for (const a in invByAgg) invByAgg[a].forEach((n, i) => { pos[n.id] = { x: (pos[a] ? pos[a].x : 0), y: maxY + ROWH * (0.86 + i * 0.6) }; });

  // place all cards
  const placed = nodes.filter(n => pos[n.id]);
  const xs = placed.map(n => pos[n.id].x), ys = placed.map(n => pos[n.id].y);
  const PAD = 34, offX = PAD - Math.min(...xs), offY = PAD - Math.min(...ys);
  const cardEls = {};
  for (const n of placed) cardEls[n.id] = placeCard(n, lane, pos[n.id].x + offX, pos[n.id].y + offY);

  // measure, size the canvas
  const box = {}; let maxR = 0, maxB = 0;
  for (const id in cardEls) {
    const e = cardEls[id];
    const b = { x: e.offsetLeft, y: e.offsetTop, w: e.offsetWidth, h: e.offsetHeight };
    b.cx = b.x + b.w / 2; b.cy = b.y + b.h / 2; box[id] = b;
    maxR = Math.max(maxR, b.x + b.w); maxB = Math.max(maxB, b.y + b.h);
  }
  lane.style.width = (maxR + PAD) + 'px';
  lane.style.height = (maxB + PAD) + 'px';

  // SVG connector layer behind the cards
  const svg = document.createElementNS(svgNS, 'svg');
  svg.setAttribute('class', 'edges');
  svg.setAttribute('width', maxR + PAD); svg.setAttribute('height', maxB + PAD);
  svg.innerHTML =
    '<defs>' +
...
function placeCard(n, lane, x, y) {
  const p = PALETTE[n.type] || PALETTE.invariant;
  if (n.type === 'invariant') {
    const c = el('div', { class: 'inv', style: 'left:' + x + 'px;top:' + y + 'px;background:' + p.fill + ';border:1px dashed ' + p.edge + ';color:' + p.text, onclick: () => openDetail(n.id) }, '⚖ ' + n.label);
    lane.append(c); return c;
  }
  const c = el('div', { class: 'sticky' + (selectedId === n.id ? ' selected' : ''), style: 'left:' + x + 'px;top:' + y + 'px;background:' + p.fill + ';border-color:' + p.edge + ';color:' + p.text, onclick: () => openDetail(n.id) },
